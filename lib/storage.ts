...
import type { SessionData, BehavioralEvent, AutopilotScore, MonkeyState, CurrentActivity } from './types';
...
function defaultSession(): SessionData {
    return {
        events: [],
        scores: [
            { window: '5m', score: 0, eventCount: 0, trend: 'stable', computedAt: Date.now() },
            { window: '25m', score: 0, eventCount: 0, trend: 'stable', computedAt: Date.now() },
            { window: '1h', score: 0, eventCount: 0, trend: 'stable', computedAt: Date.now() },
        ],
...
export const sessionItem = storage.defineItem<SessionData>('local:monkeyMeterSession', {
    fallback: defaultSession(),
});

/** Current activity — written by background on tab change, read by popup for "Currently" card */
export const currentActivityItem = storage.defineItem<CurrentActivity | null>('local:currentActivity', {
    fallback: null,
});
...
/** Save session data (prunes overflow events and logs before persisting) */
export async function saveSession(data: SessionData): Promise<void> {
    const updated = { ...data, lastUpdated: Date.now() };

    // Prune events exceeding max storage cap
    if (updated.events.length > MAX_STORED_EVENTS) {
        updated.events = updated.events.slice(-MAX_STORED_EVENTS);
    }

    // Prune activity log to prevent unbounded growth
